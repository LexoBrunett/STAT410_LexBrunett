# Create contingency table
  ...
  # Wald using function from PropCIs package
    ...
    diffpropci.Wald.mp(b=c.table[1,2], c=c.table[2,1],
                       n=sum(c.table), conf.level=0.95)
  # Wald interval using formulas
    pi.hat.table <- c.table/n
    alpha <- 0.05
    var.hat <- 1/n * ( pi.hat.plus1*(1-pi.hat.plus1) +
                       pi.hat.1plus*(1-pi.hat.1plus) -
                       2*(pi.hat.table[1,1]*pi.hat.table[2,2] -
                       pi.hat.table[1,2]*pi.hat.table[2,1]) )
    round(pi.hat.plus1 - pi.hat.1plus +
          qnorm(p=c(alpha/2, 1-alpha/2)) * sqrt(var.hat), 4)
  # Agresti and Min interval
    diffpropci.mp(b=c.table[1,2], c=c.table[2,1],
                  n=sum(c.table), conf.level=0.95)
  ...
  # New calculations
    ...
    pi.hat.1plus <- sum(c.table[1,])/n
    pi.hat.table <- c.table/sum(c.table)
    var.hat <- 1/sum(c.table) *
              ( pi.hat.plus1*(1-pi.hat.plus1) +
                pi.hat.1plus*(1-pi.hat.1plus) -
            2*(pi.hat.table[1,1]*pi.hat.table[2,2] -
               pi.hat.table[1,2]*pi.hat.table[2,1]) )
    round(sum(c.table[,1])/n  - sum(c.table[1,])/n +
            qnorm(p = c(alpha/2, 1-alpha/2)) * sqrt(var.hat), 4)
  ...
  # Tango
    scoreci.mp(b=c.table[1,2], c=c.table[2,1],
               n=sum(c.table), conf.level=0.95)
  # McNemar's test
  ...
  # BTW: depends on sample size too:
    (c.table <- 5*c.table)
    n <- sum(c.table)
    (pi.hat.plus1 <- sum(c.table[ ,1])/n)
    (pi.hat.1plus <- sum(c.table[1, ])/n)
